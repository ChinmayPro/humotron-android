...
class BloodTestViewModel @Inject constructor(
    ...
    private val _pdfResults =
        MutableLiveData<List<com.humotron.app.data.repository.ClinicalDocumentInfo>>(emptyList())
    val pdfResults: LiveData<List<com.humotron.app.data.repository.ClinicalDocumentInfo>> =
        _pdfResults
    ...
    fun searchGmail(
        ...
        viewModelScope.launch {
            ...
            try {
                val results = gmailRepository.searchPdfAttachments(
                    context,
                    accountName,
                    keywords,
                    labels,
                ...
                android.util.Log.d(
                    "BloodTestViewModel",
                    "Search completed. Found ${results.size} PDF(s)."
                )
                results.forEach {
                    android.util.Log.d(
                        "BloodTestViewModel",
                        "PDF Found: ${it.fileName} (ID: ${it.attachmentId})"
                    )
                }
            } catch (e: Exception) {
    ...
    private val _uploadState = MutableLiveData<Resource<MedicalPdfResponse>?>()
    val uploadState: LiveData<Resource<MedicalPdfResponse>?> = _uploadState
    ...
    fun setUploadResult(result: MedicalPdfResponse, initialIndex: Int = 0) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                if (file != null) {
                    medicalRepository.createClinicalDocuments(
                        ...
                        when (resource.status) {
                            com.humotron.app.data.network.Status.SUCCESS -> {
                                android.util.Log.d(
                                    "BloodTestViewModel",
                                    "uploadSelectedPdfs SUCCESS: ${resource.data?.message}"
                                )
                                _isLoading.value = false
                            }
                            ...
                            com.humotron.app.data.network.Status.ERROR -> {
                                android.util.Log.e(
                                    "BloodTestViewModel",
                                    "uploadSelectedPdfs ERROR: ${resource.error?.errorMessage}"
                                )
                                _isLoading.value = false
                            ...
                            com.humotron.app.data.network.Status.EXCEPTION -> {
                                android.util.Log.e(
                                    "BloodTestViewModel",
                                    "uploadSelectedPdfs EXCEPTION: ${resource.error?.errorMessage}"
                                )
                                _isLoading.value = false
                    ...
                    _isLoading.value = false
                    _uploadState.value =
                        Resource.error(com.humotron.app.data.network.error.Error(errorMessage = "Failed to process document"))
                }
            } catch (e: Exception) {
                _error.value = "Upload failed: ${e.message}"
                _isLoading.value = false
                _uploadState.value = Resource.error(
                    com.humotron.app.data.network.error.Error(
            ...
            medicalRepository.getAllPdfList().collect { resource ->
                when (resource.status) {
                    com.humotron.app.data.network.Status.SUCCESS -> {
                        ...
                        val metricsData = MedicalPdfData(
                            pdfData = /*convertedList*/pdfs,
                    ...
                    com.humotron.app.data.network.Status.ERROR, com.humotron.app.data.network.Status.EXCEPTION -> {
                        _uploadState.value = Resource.error(
                            resource.error
                                ?: com.humotron.app.data.network.error.Error(errorMessage = "Failed to fetch reports")
                        )
                    }
    ...
    fun generateMetricByPdfId(pdfId: String) {
        if (pdfId == lastParsedPdfId) {
            android.util.Log.d(
                "BloodTestViewModel",
                "generateMetricByPdfId: Skipping call, same PDF ID: $pdfId"
            )
            return
        }
            ...
            medicalRepository.generateMetricByPdfId(pdfId).collect { resource ->
                ...
                when (resource.status) {
                    com.humotron.app.data.network.Status.SUCCESS -> {
                        android.util.Log.d(
                            "BloodTestViewModel",
                            "generateMetricByPdfId SUCCESS: ${resource.data}"
                        )
                    }
                    ...
                    com.humotron.app.data.network.Status.ERROR -> {
                        android.util.Log.e(
                            "BloodTestViewModel",
                            "generateMetricByPdfId ERROR: ${resource.error?.errorMessage}"
                        )
                    ...
                    com.humotron.app.data.network.Status.EXCEPTION -> {
                        android.util.Log.e(
                            "BloodTestViewModel",
                            "generateMetricByPdfId EXCEPTION: ${resource.error?.errorMessage}"
                        )
                        _error.value = resource.error?.errorMessage ?: "Unexpected error"
                    }
